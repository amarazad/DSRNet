...
class TextSeqDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512, max_seq=80, seperator='[sep]'):
        assert os.path.isfile(file_path)
        directory, filename = os.path.split(file_path)
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            with open(file_path, encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        if line.lower().split(seperator)[1] == '': print(line) 
                    except:
                        print(line)
                        continue
                    ...
                    label = [-100] *  max_seq #-100 as this is the ignore index criterion for cross entropy
                    mask = [1] *  max_seq
                    ...
                    if len(tokenized_text) < max_seq:
                        ...
                        if code_str_len >= len(tokenized_text) : 
                            continue
                        code_str_len = code_str_len - ((len(tokenized_text)-max_seq))
                        
                        tokenized_text = tokenized_text[(len(tokenized_text)-max_seq):]
...
def train(args, train_dataset, model, tokenizer):
    ...
    tloss = []
    for e in tqdm(train_iterator, desc="Training epoch"):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            attn = outputs[-1] #attn added latest
            hidden_states = outputs[3] #hidden states latest
            tloss.append(loss)
            if args.n_gpu > 1:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    logger.info(f"  TRAINERR:  {(tr_loss - logging_loss)/float(args.logging_steps)}")
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(inputs, labels=labels)
            lm_loss = outputs[0]
            logger.info(f"  EVALERR:  {(lm_loss)}")
            eloss.append(lm_loss)
...
def main():
    ...
    # Setup logging
    LOG_FILENAME = args.output_dir + '.log'
    logging.basicConfig(filename=LOG_FILENAME, format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                        datefmt = '%m/%d/%Y %H:%M:%S',
    ...
    num_added_toks = tokenizer.add_tokens(['[eoq]', '[eoc]', '[eot]'])
    print('We have added', num_added_toks, ' more tokens')
    ...
    config.output_hidden_states=True #latest
    config.output_attentions=True  #latest
